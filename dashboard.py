            ...
            for idx, row in flow_df.iterrows():
                ...
                for t in threats:
                    ...
                    if ttype == "possible_port_scan" and t_src == src_ip and t_dst == dst_ip:
                        ...
                        # A DGA alert belongs to the specific DNS query, not
                        # every flow generated by the same source. Match the
                        # source, DNS destination/port, and nearby timestamp
                        # so one suspicious query cannot mark an entire
                        # browser session as malicious.
                        t_ts = _safe_float(t.get("ts"), None)
                        flow_ts = _safe_float(ts, None)
                        t_port = t.get("dst_port")
                        try:
                        ...
                        port_match = t_port_int is None or dst_port == t_port_int
                        matched = (
                            t_src == src_ip
                            and t_dst == dst_ip
                            and port_match
